...
# Parse ADMINS from config
ADMINS = list(map(int, ADMINS.split())) if ADMINS else []
if not ADMINS:
...
async def start_command(client, message):
    ...
    if verify_status["is_verified"] or user_id in ADMINS:
        logging.info(f"User {user_id} is verified or admin")
        reply_message = (
...
async def handle_message(client, message: Message):
    ...
    if not verify_status["is_verified"] and user_id not in ADMINS:
        await message.reply_text("To use this bot, please verify your identity. Click /start to begin.")
        return

    # Skip force subscribe check for admins
    ...
    try:
        ...
        if file_path:  # Only upload if download was successful
            is_admin = user_id in ADMINS
            await upload_video(client, file_path, thumbnail_path, video_title, reply_msg, dump_id, user_mention, user_id, message, is_admin)
    except Exception as e:
